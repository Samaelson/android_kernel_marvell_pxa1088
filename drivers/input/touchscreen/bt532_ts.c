...
static void get_module_vendor(void *device_data)
{
	...
	case 2:
		...
		break;
	default: 
		module_vendor = TSP_HW_ID_INDEX_NO_LCD;
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "%s,%d", "OK", module_vendor);  //no lcd
		break;
	}
	set_cmd_result(info, finfo->cmd_buff,
					strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	...
	dev_info(&client->dev, "%s: %s(%d) : %d\n", __func__, finfo->cmd_buff,
				strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)), module_vendor);
